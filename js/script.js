...
function scene() {
   ...
   //  gameCam.attachControl(canvas); <-- Disabling player control of camera. Now I can be lazy
    ...
    var endFieldComputer = new BABYLON.Mesh.CreateGround("endFieldComputer", 9.25, 1, 1,  gameScene);
    endFieldComputer.position.z = 15;
    var endFieldComputerMat = new BABYLON.StandardMaterial("endFieldComputerMat", gameScene);
    endFieldComputerMat.diffuseColor = new BABYLON.Color3(1.0, 0, 0);
    endFieldComputer.material = endFieldComputerMat;
    endFieldComputer.receiveShadows = true;
    ...
    wall1.scaling.y = 1;
    wall1.scaling.z = 22;
    wall1.position = new BABYLON.Vector3(5, 0.5, 4.5);
    ...
    wall2.scaling.y = 1;
    wall2.scaling.z = 22;
    wall2.position = new BABYLON.Vector3(-5, 0.5, 4.5);
    ...
    // AI paddle
    var computerPaddle = BABYLON.Mesh.CreateBox("paddleAI", 1, gameScene);
    var computerPaddleMat = new BABYLON.StandardMaterial("computerMat", gameScene);
    computerPaddleMat.diffuseColor = new BABYLON.Color3(0.7, 1.0, 0.5);
    computerPaddle.material = computerPaddleMat;
    computerPaddle.scaling.x = 1.5;
    computerPaddle.scaling.y = 0.35;
    computerPaddle.scaling.z = 0.5;
    computerPaddle.position = new BABYLON.Vector3(0, 0.45, 14);
    
    shadowGenerator.getShadowMap().renderList.push(computerPaddle);
    
    function aiMovement() {
        computerPaddle.position.x = ballObject.position.x;
    }
    ...
    function ballMovement () {
        ...
        && ballObject.position.x >= playerPaddle.position.x - playerPaddle.scaling.x) {
            ...
            && ballObject.position.z >= playerPaddle.position.z - playerPaddle.scaling.z) {
                ...
                if (ballDirZ < 0) {
                    ...
                    ballDirZ -= ballReflectDist * (playerPaddle.position.x - ballObject.position.x);

                    // Reflect ball based on position of collision with paddle

                    console.log(ballObject.position.x - playerPaddle.position.x);
        ...
        // Check if ball is aligned with AI paddle on X-axis
        if (ballObject.position.x <= computerPaddle.position.x + computerPaddle.scaling.x
        && ballObject.position.x >= computerPaddle.position.x - computerPaddle.scaling.x) {
            // Check if ball is aligned with AI paddle on Z-axis
            if (ballObject.position.z <= computerPaddle.position.z + computerPaddle.scaling.z
            && ballObject.position.z >= computerPaddle.position.z - computerPaddle.scaling.z) {
                // Check if ball is traveling towards AI
                if(ballDirZ > 0) {
                    // Switch direction of ball
                    ballDirZ = -ballDirZ;
                    // Impact angle
                    ballDirZ -= ballReflectDist * Math.random();    
